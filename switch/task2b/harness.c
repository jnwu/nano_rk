...
/*define the variable holding total time of delay. This is used to calculate the 
average time delay for each packet later */
...
static BOOL receive_message_if_there_is_one(int output_port)
{
   ...
   if (out_port[output_port].flag) {
	...
	/*obtain the the packet's time delay. Add the value to the total time variable*/

	struct timeval currentTime;
	gettimeofday(&currentTime, NULL);
	double ct = currentTime.tv_sec + (currentTime.tv_usec/1000000.0);
	double st = packet.sendTime.tv_sec + (packet.sendTime.tv_usec/1000000.0);
	totalTime = totalTime + ct - st;
